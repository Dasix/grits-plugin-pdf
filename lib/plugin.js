...
pr.onDustRender = function( renderer, ev ) {
	...
	if( isWriteable === true ) {
		...
		// Render the output..
		me._doPhantomRender( src, dest, opts );
 ...
 * @return {void}
 */
pr._doPhantomRender = function( src, dest, opts ) {

	var me 		= this;
	var html	= fs.readFileSync( src.getAbsoluteFilePath(), "utf8" );

	function afterPhantomRender( err, res ) {
	...
	// Runs Phantom.js
	phantomWrapper.create( html, opts ).toFile( dest.getAbsoluteFilePath(), afterPhantomRender );
